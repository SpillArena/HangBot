    ...
    return (
      <main className={shellClassName}>
        <section className={`${panelClassName} p-6 text-center`}>
          ...
          <p
            className={`mt-3 text-sm ${isLightTheme ? 'text-slate-700' : 'text-slate-200'}`}
          >
            {t('game.unableInitRound')}
          </p>
      ...
      >
        ...
        >
          <div>
            ...
            <h1
              className={`text-xl font-semibold ${isLightTheme ? 'text-slate-900' : 'text-white'}`}
            >
              {username}
            </h1>
          </div>
        ...
        <section className="grid gap-6 lg:grid-cols-[300px_minmax(0,1fr)]">
          <aside className={`${panelClassName} p-4`}>
            <p
              className={`mb-1 text-xs uppercase tracking-[0.2em] ${isLightTheme ? 'text-slate-500' : 'text-slate-400'}`}
            >
            ...
            >
              ...
              <li>
                {t('game.category')}: {round.category}
              </li>
            </ul>
            ...
            <div
              className={
                isLightTheme
                  ? 'mt-3 rounded-xl border border-slate-300 bg-slate-50 p-3'
                  : 'mt-3 rounded-xl border border-slate-800 bg-slate-900/80 p-3'
              }
            >
              <p
                className={`text-xs uppercase tracking-wide ${isLightTheme ? 'text-slate-500' : 'text-slate-400'}`}
              >
                {t('game.hint')}
              </p>
              <p
            ...
            <pre
              className={
                isLightTheme
                  ? 'mt-4 overflow-auto rounded-xl border border-slate-300 bg-slate-50 px-3 py-3 text-xs text-slate-700'
                  : 'mt-4 overflow-auto rounded-xl border border-slate-800 bg-slate-900 px-3 py-3 text-xs text-slate-200'
              }
            >
          ...
          <div className={`${panelClassName} p-5`}>
            <div
              className={
                isLightTheme
                  ? 'rounded-xl border border-slate-300 bg-slate-50 px-4 py-5'
                  : 'rounded-xl border border-slate-800 bg-slate-900/80 px-4 py-5'
              }
            >
              <p
                className={`mb-2 text-xs uppercase tracking-[0.2em] ${isLightTheme ? 'text-slate-500' : 'text-slate-400'}`}
              >
                {t('game.mysteryWord')}
              </p>
              <p
                className={`break-words font-mono text-2xl font-semibold tracking-[0.35em] sm:text-3xl ${isLightTheme ? 'text-slate-900' : 'text-white'}`}
              >
                {maskedWord}
              </p>
            </div>
                ...
                return (
                  <button
                    ...
                    className={[
                      ...
                      !hasGuessed
                        ? isLightTheme
                          ? 'border-slate-300 bg-white text-slate-800 hover:border-cyan-500 hover:text-cyan-700'
            ...
            <div
              className={
                isLightTheme
                  ? 'mt-4 rounded-xl border border-slate-300 bg-slate-50 p-3 text-xs text-slate-600'
                  : 'mt-4 rounded-xl border border-slate-800 bg-slate-900/60 p-3 text-xs text-slate-400'
              }
            >
              {t('game.keyboardTip', {
                alphabetLabel: language === 'no' ? 'A-Å' : 'A-Z',
              })}
            </div>
              ...
              >
                ...
                <div className="mt-4 flex flex-wrap gap-2">
                  ...
                  <button
                    ...
                    className={
                      isLightTheme
                        ? 'rounded-lg border border-slate-300 bg-white px-3 py-2 text-xs font-semibold uppercase tracking-wide text-slate-700 transition hover:border-cyan-500 hover:text-cyan-700'
                        : 'rounded-lg border border-slate-600 px-3 py-2 text-xs font-semibold uppercase tracking-wide text-slate-200 transition hover:border-cyan-400 hover:text-cyan-200'
                    }
                  >
                    {t('game.returnLobby')}
                  </button>
